...
export default function UserProfilePage() {
  ...
  useEffect(() => {
    ...
    if (storedProfile) {
      ...
      setReferralCode(`CROMA-${profile.ethAddress.slice(2, 8).toUpperCase()}`)
      setIsDemoMode(false)
    } else {
      const demoProfile: UserProfile = {
        name: "Demo User",
        email: "demo.user@example.com",
        telegramUsername: "@demo_telegram",
        discordUsername: "demo_discord#1234",
        twitterUsername: "@demo_twitter",
        ethAddress: "0xDemoAddress123456789012345678901234567890",
      }
      setUserProfile(demoProfile)
      setReferralCode(`CROMA-${demoProfile.ethAddress.slice(2, 8).toUpperCase()}`)
      setIsDemoMode(true)
    }
  ...
  const handleSaveProfile = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.updatedProfile) {
        ...
        localStorage.setItem("userProfile", JSON.stringify(result.updatedProfile))
        setProfileUpdateMessage({ type: "success", description: result.message })
        setShowEditProfileDialog(false)
      } else {
        setProfileUpdateMessage({ type: "error", description: result.message || "Failed to update profile." })
      }
    } catch (error) {
      console.error("Error saving profile:", error)
      setProfileUpdateMessage({ type: "error", description: "An unexpected error occurred. Please try again." })
    } finally {
  ...
  return (
    <div className="min-h-screen text-white overflow-x-hidden flex flex-col">
      ...
      {/* Profile Section */}
      <section ref={profileRef} className="relative min-h-screen flex items-center justify-center px-4 py-20 pt-24">
        <motion.div
            ...
            <motion.div variants={fadeInUp}>
              <Alert className="bg-yellow-900/20 border-yellow-500/30 max-w-2xl mx-auto">
                ...
                <AlertDescription className="text-yellow-200">
                  <strong>Not Registered:</strong> Please register or sign in to view your profile and progress.
                  <Button
                ...
                >
                  <Alert className="bg-blue-900/20 border-blue-500/30 max-w-2xl mx-auto mb-4">
                    ...
                    <AlertDescription className="text-blue-200">
                      You are viewing a demo profile. To save your own data, please register or sign in. To return to
                      demo mode, clear site data from your browser localStorage.
                    </AlertDescription>
                  </Alert>
                </motion.div>
              )}
              ...
              {profileUpdateMessage && (
                ...
                >
                  ...
                  >
                    ...
                    <AlertDescription
                      className={`${profileUpdateMessage.type === "success" ? "text-green-200" : "text-red-200"}`}
                    >
                ...
                <Card className="bg-black/40 backdrop-blur-md border border-orange-500/30 p-6 md:p-8 text-left">
                  ...
                  <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-300">
                    <div className="flex items-center space-x-2">
                      ...
                      <span>
                        Name: <span className="font-semibold text-white">{userProfile.name}</span>
                      </span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Mail className="w-5 h-5 text-orange-500" />
                      <span>
                        Email: <span className="font-semibold text-white">{userProfile.email}</span>
                      </span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <MessageCircle className="w-5 h-5 text-orange-500" />
                      <span>
                        Telegram: <span className="font-semibold text-white">{userProfile.telegramUsername}</span>
                      </span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <MessageCircle className="w-5 h-5 text-orange-500" />
                      <span>
                        Discord: <span className="font-semibold text-white">{userProfile.discordUsername}</span>
                      </span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Twitter className="w-5 h-5 text-orange-500" />
                      <span>
                        Twitter/X: <span className="font-semibold text-white">{userProfile.twitterUsername}</span>
                      </span>
                    </div>
                    <div className="flex items-center space-x-2">
                      ...
                      <span>
                        ETH Address:{" "}
                        <span className="font-semibold text-white break-all">{userProfile.ethAddress}</span>
                      </span>
                    </div>
                  </CardContent>
                </Card>
              ...
              {referralCode && (
                <motion.div variants={fadeInUp}>
                  <Card className="bg-black/40 backdrop-blur-md border border-green-500/30 p-6 md:p-8 text-left">
                    ...
                    <p className="text-gray-400 mt-4">Share this link to earn bonus CROMA tokens!</p>
                  </Card>
                </motion.div>
              )}
                ...
                <Card className="bg-black/40 backdrop-blur-md border border-orange-500/30 p-6 md:p-8 text-left">
                  ...
                  <div className="space-y-4">
                    {tasks.map((task) => {
                      ...
                      return (
                        <div key={task.id} className="flex items-center justify-between p-3 rounded-lg bg-gray-900/30">
                          <div className="flex items-center space-x-3">
                            ...
                            <span
                              className={`font-semibold ${task.status === "completed" ? "text-green-400" : "text-white"}`}
                            >
                              {task.title}
                            </span>
                          </div>
        ...
        <DialogContent className="bg-black/90 border border-orange-500/30 text-white max-w-2xl">
          ...
          {profileUpdateMessage && profileUpdateMessage.type === "error" && (
            <Alert className="bg-red-900/20 border-red-500/30">
              <AlertTriangle className="h-4 w-4 text-red-500" />
              <AlertDescription className="text-red-200">{profileUpdateMessage.description}</AlertDescription>
            </Alert>
          )}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => handleEditInputChange("telegramUsername", e.target.value)}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => handleEditInputChange("discordUsername", e.target.value)}
                  className="bg-gray-900/50 border-orange-500/30 text-white focus:border-orange-500"
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => handleEditInputChange("twitterUsername", e.target.value)}
                  className="bg-gray-900/50 border-orange-500/30 text-white focus:border-orange-500"
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => handleEditInputChange("ethAddress", e.target.value)}
                  className="bg-gray-900/50 border-orange-500/30 text-white focus:border-orange-500"
                />
              </div>
